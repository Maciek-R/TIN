...
void TicketServer::AnswerOnRequestForTicket(bool isClientAuthorized, unsigned char idService)
{
	...
	if(isClientAuthorized)
	{
		...
		encryptedTicket[0] = serviceInfo[0];
		AES_encrypt(serviceInfo+1, encryptedTicket+1, &encryptionKey);

		if(sendto(mainSocket, encryptedTicket, 46, 0, (struct sockaddr *) &address, sizeof(address)) != 46)
		{
...
void TicketServer::LoadServiceInfo(bool isClientAuthorized, unsigned char idService)
{
	...
	if(isClientAuthorized)
	{
		...
		unsigned char serializedTicket[45];
		createdTicket.Serialize(serializedTicket);
		
		for(size_t i = 0; i < 45; ++i)
		{
			mess[i+1] = serializedTicket[i];
		}
